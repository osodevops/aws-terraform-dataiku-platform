...
@mock.patch('src.volume.Volume._get_volumes', autospec=True)
def test_volume(mock_get_volumes):
    aws_obj = MockAws()
...
def test_get_volumes_no_data(mock_get_latest_volume):
    ...
    assert not volume_obj.az
    assert not volume_obj.vol_exists


@mock.patch('src.volume.Volume._get_volumes', autospec=True)
def test_get_latest_volume(mock_get_volumes):
    aws_obj = MockAws()
    volume_obj = Volume(mock_event_data, aws_obj, {})
    data = [
        mock_volume_younger,
...
@mock.patch('src.volume.Volume._get_volumes', autospec=True)
def test_get_latest_volume_no_data(mock_get_volumes):
    aws_obj = MockAws()
    volume_obj = Volume(mock_event_data, aws_obj, {})
